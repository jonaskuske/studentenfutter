  <div class="flex flex-col items-center px-8 pt-4 pb-10 bg-white shadow rounded-large">
    <h2 class="mb-8 text-xl italic font-bold leading-loose highlight highlight-blue">Suche</h2>

    <form class="w-full group" action="<?= $site->find('search')->url() ?>" method="GET">
      <div class="flex border-b-2 border-dotted text-blue">
        <svg class="mr-2 fill-current" width="23" height="23" viewBox="0 0 23 23" xmlns="http://www.w3.org/2000/svg">
        ...
        <input class="w-full placeholder-blue focus:outline-none" type="search" name="q" required value="<?= html(get('q')) ?>" placeholder="z.B. Reispfanne" aria-label="Suche" />

        <button type="submit" class="transition-opacity duration-200 opacity-0 group-valid:opacity-100">
