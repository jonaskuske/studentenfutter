  <div class="flex flex-col items-center px-8 pt-4 pb-10 bg-white shadow rounded-card">
    ...
    <form x-data="{ query: '' }" class="w-full" action="<?= $site->find('search')->url() ?>">
      <div class="flex max-w-sm mx-auto border-dotted border-b-6 border-circles-blue text-blue">
        ...
        <input value="<?= html(get('q')) ?>" class="w-full placeholder-blue focus:outline-none" type="search" name="q" placeholder="z.B. Reispfanne" aria-label="Suche" @input="query = $event.target.value" />
